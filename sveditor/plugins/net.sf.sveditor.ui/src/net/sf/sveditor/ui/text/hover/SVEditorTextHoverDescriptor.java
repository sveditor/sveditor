 ...
 * @since 2.1
 */
public class SVEditorTextHoverDescriptor {
...
//	private boolean fIsEnabled;
	 ...
	 */
	public static SVEditorTextHoverDescriptor[] getContributedHovers() {
		IExtensionRegistry registry= Platform.getExtensionRegistry();
		IConfigurationElement[] elements= registry.getConfigurationElementsFor(JAVA_EDITOR_TEXT_HOVER_EXTENSION_POINT);
		SVEditorTextHoverDescriptor[] hoverDescs= createDescriptors(elements);
...
//	}
	 ...
	 */
	private SVEditorTextHoverDescriptor(IConfigurationElement element) {
		Assert.isNotNull(element);
		fElement= element;
		fLog = LogFactory.getLogHandle("HoverDescriptor") ;
	}
	 ...
	 */
	public ISVEditorTextHover createTextHover() {
		...
		if (isHoversPlugInActivated || canActivatePlugIn()) {
			try {
				return (ISVEditorTextHover)fElement.createExecutableExtension(CLASS_ATTRIBUTE);
			} catch (CoreException x) {
//				SVUiPlugin.log(new Status(IStatus.ERROR, SVUiPlugin.PLUGIN_ID, 0, SVHoverMessages.JavaTextHover_createTextHover, null));
				fLog.error("create text hover failed", x) ;
	...
	public boolean equals(Object obj) {
		if (obj == null || !obj.getClass().equals(this.getClass()) || getId() == null)
			return false;
		return getId().equals(((SVEditorTextHoverDescriptor)obj).getId());
	}
	...
	private static SVEditorTextHoverDescriptor[] createDescriptors(IConfigurationElement[] elements) {
		List<SVEditorTextHoverDescriptor> result= new ArrayList<SVEditorTextHoverDescriptor>(elements.length);
		for (int i= 0; i < elements.length; i++) {
			IConfigurationElement element= elements[i];
			if (HOVER_TAG.equals(element.getName())) {
				SVEditorTextHoverDescriptor desc= new SVEditorTextHoverDescriptor(element);
				result.add(desc);
			}
		}
		return result.toArray(new SVEditorTextHoverDescriptor[result.size()]);
	}

	private static void initializeFromPreferences(SVEditorTextHoverDescriptor[] hovers) {
//		String compiledTextHoverModifiers= SVUiPlugin.getDefault().getPreferenceStore().getString(PreferenceConstants.EDITOR_TEXT_HOVER_MODIFIERS);
//
...
//		String compiledTextHoverModifiers= SVUiPlugin.getDefault().getPreferenceStore().getString(PreferenceConstants.EDITOR_TEXT_HOVER_MODIFIER_MASKS);
//
